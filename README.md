...
Если игра не закончена, Вы считываете двумерный массив клеток размера `height` * `width`. Это та часть карты, которую видит ваш бот. Каждая клетка представляет собой 6 переменных.
...
* MOVE
  ...
  `amount -> integer` - кол-во войск, которое вы желаете передвинуть.
...
В локальной папке `/binaries/` есть файлы `1`, `2`, `3`, `4`. В них происходит вывод из std::cerr внутри программ 1, 2, 3 и 4-го игроков соответственно. Сответственно, для отладки ботов можно использовать вывод в поток std::cerr.
